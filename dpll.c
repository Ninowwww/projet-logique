...
clauses_grille_solution algo_dpll(int** clauses, int nb_clauses, int dimension) {
    ...
    *(clauses_grille.validite) = 1;
    ...
    // printf("########## fin dpll ################\n");
    algo_dpll_rec(&clauses_grille, nb_clauses, dimension,0);
...
void algo_dpll_rec(clauses_grille_solution * clauses_grille, int nb_clauses, int dimension, int prof ) {
  printf("########## deb rec %d ################\n", prof);
    ...
    if (*(clauses_grille->validite) == 0) {     // Si toutes les clauses sont toutes vides on renvoie ce résultat, on a résolue l'ensemble
      ...
      printf("########## fin 5 rec %d ################\n", prof);
      printf("     => Ensemble des clauses vide, ensemble satisfaisable !!! %d \n", *clauses_grille->validite);
      ...
      printf(" je rentre dans la branche de gauche a profondeur %d ################\n", prof);

        assigner_valeur_litteral(*clauses_grille, clauses_grille->clauses[idx_clause_non_vide][0], nb_clauses, dimension);
        algo_dpll_rec(clauses_grille, nb_clauses, dimension, prof+1);
	printf(" prof=%d, clauses_grille->validite=%d\n", prof, *clauses_grille->validite);
	  ...
	  *clauses_grille = copier_clauses_grille(copie1, nb_clauses, dimension);
	  ...
	  return;
        ...
        *(clauses_grille->validite) = 0;
        printf("Modele non valide à profindeur %d!! \n", prof);
      return;
